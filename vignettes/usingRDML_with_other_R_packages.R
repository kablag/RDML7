...
## -----------------------------------------------------------------------------
names(raw_data$experiment[["Standard Curve Example"]]$run$Run001)
raw_data$experiment[["Standard Curve Example"]]$run$Run001$instrument
raw_data$experiment[["Standard Curve Example"]]$run$Run001$pcrFormat

## -----------------------------------------------------------------------------
fdata <- 
  asTable(raw_data,
          # Custom name pattern
          namePattern = "{position}:{sample}~{sampleType}~{target}") |> 
  # Get all fluorescence data
  getFData(raw_data, request = _, long.table=FALSE)
...
res_fit <- do.call(cbind, lapply(2L:ncol(fdata), function(block_column) {
    ...
    if(class(res_model)[1] == "try-error") {
        res_model <- NA
    }
    else{
...
Cq_SDM <- res_fit[rownames(res_fit)=="cpD2", ] |> 
  unlist() |> 
  as.data.frame()
...
dilution <- c(as.factor("ntc"), as.factor("unk"), as.factor("unk"),
...
sample_Cq <- data.frame(sample=c("ntc", "unk", "unk", 
                                 10000, 5000, 2500, 1250, 625), 
                        Cq_values)
