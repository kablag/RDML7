---
title: "Using RDML7 with other R packages"
...
# A Practical Example for Usage of the **RDML7** package

A key benefit of the **RDML7** package is that it enables further statistical
analysis of RDML data. This section describes **R** packages 
that link with the **RDML7** package. It will provide examples on how
...
from that of the proposed pipeline (**RDML7** $\rightarrow$ **qpcR** $\rightarrow$ **chipPCR**).
...
In the **RDML7** vignette, it was demonstrated how to read-in RDML data. In this section we will 
continue with the built-in RDML example file `stepone_std.rdml`. The file is opened as 
described before:
...
For convenience, we use the built in pipe function `|>` for 
further analysis. In the following, we fetch the amplification curve data from
the RDML file.
...
average Cq of `r paste(round(mean(as.numeric(sample_Cq[2, -1])), 2), "±", round(sd(sample_Cq[2, -1]), 2))`.
